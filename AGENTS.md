...
- 섹션과 비중은 `config/sections.json`에 있다. 모든 섹션 최소 목표 3건, 각 섹션 최대 9건(총 최대 72건). 같은 사건을 다룬 기사는 유사 기사 묶기로 대표 카드 아래에 붙이므로 카드 9건은 서로 다른 사건이어야 한다. 무료·발행일 검증 통과 후보 부족 시 미달을 JSON에 기록하고 기준을 완화하지 않는다.
...
- 요약 문장 점수의 키워드 가점은 가중치가 양수인 키워드만 쓴다(음수 키워드는 제목 순위에서만 후순위로 작용).
- 유사 기사 판정은 제목 기준이다: `[…]`·괄호 제거 후 글자 2-gram Dice ≥ 0.45, 숫자 집합이 같거나 한쪽이 비어 있고, 방향어 쌍(상승/하락·증가/감소·매수/매도·흑자/적자·인상/인하·확대/축소·급등/급락·강세/약세·증설/축소·상향/하향·호조/부진·반등/하락)이 어긋나지 않을 때 같은 사건으로 본다. 관련 기사는 카드당 4건, 한도 이후 추가 본문 접근은 섹션당 8건까지다. 관련 기사도 무료·발행 시각·기간 검증을 통과해야 한다. 요약 점수는 순위에 반영하지 않는다.
...
- UI에는 날짜, 검색, 섹션, 뉴스 요약, 수집 버튼, 수집 진행률, 오늘 보고서 없음 배너, 미수집 날짜 재수집, 헤드라인 리본, 읽음 표시·북마크(저장함), 키워드 강조(제목·요약), 관련 보도 토글, 기간·언론사 필터, 최근 7일 모아보기, 14일 기사 수 막대, 테마 전환(시스템/라이트/다크), 키워드 설정 메뉴만 둔다. 출처 상태·제외 통계·안내 카드 등은 추가하지 않는다. 진단 정보는 JSON에만 남긴다. 읽음·북마크·테마 선택은 브라우저 localStorage에만 저장한다.
- PDF 보고서는 만들지 않는다.
