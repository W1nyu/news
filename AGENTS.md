...
- UI에는 날짜, 검색, 섹션, 뉴스 요약, 수집 버튼, 수집 진행률, 오늘 보고서 없음 배너, 미수집 날짜 재수집, 헤드라인 리본, 읽음 표시·북마크(저장함), 키워드 강조(제목·요약), 관련 보도 토글, 기간·언론사 필터, 키워드 설정 메뉴만 둔다. 출처 상태·제외 통계·안내 카드 등은 추가하지 않는다. 진단 정보는 JSON에만 남긴다. 읽음·북마크는 브라우저 localStorage에만 저장한다.
- PDF 보고서는 만들지 않는다.
