...
- **반응형·다크 모드**: 스타일은 당근 SEED 디자인 토큰(`dist/tokens.css`, 브랜드 색만 딥그린)과 `dist/app.css`로 구성됩니다. 모바일(<768px)은 고정 상단바·가로 스크롤 섹션 탭·바텀시트 설정 화면, 태블릿 2열, 데스크톱(≥1280px) 3열이며 다크 모드는 시스템 설정을 따릅니다.
